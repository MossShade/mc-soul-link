...
public class Health implements Command<ServerCommandSource> {

	public static void register(LiteralArgumentBuilder<ServerCommandSource> root) {
		root.then(CommandManager.literal(Constants.COMMAND_CONFIG_HEALTH)
	...
	public int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
		...
		ConfigManager.CONFIG.shared_health = enabled;
		ConfigManager.save();

		serverCommandSource.sendFeedback(() -> Text.translatable(Constants.CONFIG_HEALTH_MESSAGE, Config.getEnableStatusMessage(enabled)), false);
