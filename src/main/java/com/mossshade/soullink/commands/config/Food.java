...
public class Food implements Command<ServerCommandSource> {

	public static void register(LiteralArgumentBuilder<ServerCommandSource> root) {
		root.then(CommandManager.literal(Constants.COMMAND_CONFIG_FOOD)
	...
	public int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
		...
		ConfigManager.CONFIG.shared_food = enabled;
		ConfigManager.save();

		serverCommandSource.sendFeedback(() -> Text.translatable(Constants.CONFIG_FOOD_MESSAGE, Config.getEnableStatusMessage(enabled)), false);
